...
@pytest.mark.parametrize("error_message, expected_details", [
    ("(psycopg2.errors.ForeignKeyViolation) insert or update on table \"PersonPhone\" violates foreign key constraint "
     ...
     dict(line="DETAIL:  Key (PhoneNumberTypeID)=(10) is not present in table \"PhoneNumberType\".",
    ...
    ("(psycopg2.errors.ForeignKeyViolation) insert or update on table \"PersonPhone\" violates foreign key constraint "
     ...
     dict(line="DETAIL:  Key (BusinessEntityID)=(20786) is not present in table \"PhoneNumberType\".",
